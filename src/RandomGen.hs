...
generateLevelFloorBlocks n mDist = do
                                ...
                                return blockList
                                    where   add xs ys   = zipWith (+) xs ys
                                            mult xs ys  = zipWith (*) xs ys

updateFloorBlocks' :: LevelMap -> [FloorBlock] -> LevelMap
updateFloorBlocks' lev bs = lev {floorBlocks = bs}

updateFloorBlocks :: GameState -> [FloorBlock] -> GameState
updateFloorBlocks gstate bs = gstate {level = updateFloorBlocks' (level gstate) bs}
